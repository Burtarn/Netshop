...
router.get('/', (req, res) => {
    ...
    fs.readFile(filePath, 'utf8', (err, data) => {
        ...
        try {
            ...
            const trojor = jsonData?.data?.products?.trojor;
            if (!trojor) {
                console.error('trojor-data hittades inte!');
                return res.status(404).json({ message: 'trojor-data hittades inte.' });
            }

            console.log('trojor-data:', trojor);
            res.json(trojor);
        } catch (parseError) {
