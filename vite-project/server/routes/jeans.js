...
router.get('/', (req, res) => {
    ...
    fs.readFile(filePath, 'utf8', (err, data) => {
        ...
        try {
            ...
            const jeans = jsonData?.data?.products?.jeans;
            if (!jeans) {
                console.error('Jeans-data hittades inte!');
                return res.status(404).json({ message: 'Jeans-data hittades inte.' });
            }

            console.log('Jeans-data:', jeans);
            res.json(jeans);
        } catch (parseError) {
